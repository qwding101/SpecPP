...
names(KSDE)
```
Note that the KSDE for cross pseudo-spectrum is complex-valued. To visualize the result, we need to plot the real and imaginary parts separately.
```{r}
KSDE.real = lapply(KSDE, Re)
KSDE.im = lapply(KSDE, Im)
```
